...
/**
 * Unique identifier for an {@link Incident}.
 * <p>
    ...
    public IncidentId {
        ...
        if (value.length() != NANO_ID_LENGTH) {
            throw new IllegalArgumentException(
                    "IncidentId must be exactly " + NANO_ID_LENGTH + " characters, got: " + value.length());
        }
        for (int i = 0; i < value.length(); i++) {
            if (!isNanoIdChar(value.charAt(i))) {
                throw new IllegalArgumentException(
                        "IncidentId contains invalid character at position " + i + ": " + value);
            }
        }
    }
