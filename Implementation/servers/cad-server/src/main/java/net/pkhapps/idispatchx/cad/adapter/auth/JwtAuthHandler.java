 ...
 * <p>
 * Extracts the Bearer token, validates it using {@link TokenValidator}, checks revocation via
 * {@link SessionStore}, and stores validated claims in the request context via {@link AuthContext}.
 * Responds with HTTP 401 if the token is missing, expired, or invalid.
 */
    ...
    public void handle(Context ctx) throws Exception {
        ...
        try {
            ...
            if (sessionStore != null && claims.sessionId() != null
                    && sessionStore.isRevoked(claims.sessionId())) {
                log.debug("Session {} has been revoked for subject: {}", claims.sessionId(), claims.subject());
            ...
            log.debug("Token validation failed for {}: {}", ctx.path(), e.getMessage());
