...
class EventBroadcasterTest {
    ...
    /** Records each method call on the broadcast service (what type + seq + payload). */
    ...
    void setUp() {
        ...
        // Create a DispatcherBroadcastService that records calls instead of sending to sessions.
        // We override the individual send*() methods on the non-final class.
        var recordingService = new DispatcherBroadcastService(new SessionRegistry()) {
            @Override
            public void sendCallCreated(long seq, Instant ts, Object p) {
                broadcastCalls.add(new BroadcastCall("call.created", seq, ts, p));
            }

            @Override
            public void sendCallUpdated(long seq, Instant ts, Object p) {
                broadcastCalls.add(new BroadcastCall("call.updated", seq, ts, p));
            }

            @Override
            public void sendCallEnded(long seq, Instant ts, Object p) {
                broadcastCalls.add(new BroadcastCall("call.ended", seq, ts, p));
            }

            @Override
            public void sendCallAttachedToIncident(long seq, Instant ts, String callId, String incidentId) {
                broadcastCalls.add(new BroadcastCall("call.attached_to_incident", seq, ts,
                        Map.of("callId", callId, "incidentId", incidentId)));
            }

            @Override
            public void sendCallDetachedFromIncident(long seq, Instant ts, String callId, String formerIncidentId) {
            ...
            @Override
            public void sendIncidentCreated(long seq, Instant ts, Object p) {
                broadcastCalls.add(new BroadcastCall("incident.created", seq, ts, p));
            }

            @Override
            public void sendIncidentLogEntryAdded(long seq, Instant ts, Object p) {
                broadcastCalls.add(new BroadcastCall("incident.log_entry_added", seq, ts, p));
            }
        };

        broadcaster = new EventBroadcaster(recordingService);
    }
    ...
    @Test
    void broadcast_callAttachedToIncidentEvent_containsCallAndIncidentIds() {
    ...
    void broadcast_unknownEvent_producesNoMessage() {
        ...
        broadcaster.broadcast(event, 99L);
        assertTrue(broadcastCalls.isEmpty());
    }

    @Test
    void broadcast_sequenceNumberIsIncludedCorrectly() {
        var event = new CallCreatedEvent(EventId.generate(), NOW, CommandId.generate(), DISPATCHER,
                CALL_ID, null, null, null, null);
    ...
    // -----------------------------------------------------------------------
    // Test double for an unrecognized event type
    // -----------------------------------------------------------------------
